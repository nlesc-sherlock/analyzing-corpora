...
class clustering:
  ...
  def data_embedding(self, type='TSNE'):
      ...
      pos = model.fit(self.distance_matrix).embedding_  # position of points in embedding space
      return pos
  ...
  def explained_variance(self, nclusters, filename='elbow.pdf'):
      ...
      centers = [KMeans(n_clusters=k).fit(self.distance_matrix).cluster_centers_ for k in KK]
      D_k = [cdist(self.distance_matrix, cent, 'euclidean') for cent in centers]
      dist = [numpy.min(D,axis=1) for D in D_k]
      # Total within-cluster sum of squares
      tot_withinss = [sum(d**2) for d in dist]
      # The total sum of squares
      totss = sum(pdist(self.distance_matrix)**2)/self.distance_matrix.shape[0]
  ...
  def topic_weights(self, cluster, nwords=25):
      ...
      topic_out = self.find_topics_per_cluster(self.topics, self.k_fit, cluster)
