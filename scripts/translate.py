...
class translate:
    ...
    def _new_database(self):
        ...
        '''
        # TODO: remove next two lines after testing -> don't automatically
        # remove
        ...
        if os.path.exists(self.database):
            ...
            logger.info('Database not found, creating database %s' %
                        self.database)
            try:
    ...
    def _create_dbstructure(self):
        ...
        '''
        # self.connection.row_factory = sqlite3.Row  # use dict instread of
        # tuple
        self.cursor = self.connection.cursor()
        # create table
        self.cursor.execute('''CREATE TABLE filelist
                           (filename TEXT, processed BOOLEAN)''')

    def _add_files_to_database(self):
        '''
        add filenames to database table
        '''
        list_files = numpy.transpose(numpy.vstack((numpy.transpose(self.files),
    ...
    def _translate_text(self, dest_lang='nl'):
        ...
        '''
        try:
            self.translated_text = self.translate.translate(
                self.text, dest_lang)
